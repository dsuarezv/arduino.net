...
namespace AurelienRibon.Ui.SyntaxHighlightBox
{
    ...
    {
        private static HighlighterManager mInstance;
        private static XmlReaderSettings mXmlSchemaSettings;

        public static HighlighterManager Instance 
        { 
            get 
            { 
                if (mInstance == null) mInstance = new HighlighterManager(); 
        ...
        private HighlighterManager()
        {
            ...
            RegisterDefinitions(this.GetType().Assembly);
        }

        public void RegisterDefinitions(Assembly ass)
        {
            if (mXmlSchemaSettings == null)
            {
                var highlighterSchema = Application.GetResourceStream(new Uri("pack://application:,,,/AurelienRibon.Ui.SyntaxHighlightBox;component/resources/syntax.xsd"));
                var schemaStream = highlighterSchema.Stream;
                XmlSchema schema = XmlSchema.Read(schemaStream, (s, e) =>
                {
                    Debug.WriteLine("Xml schema validation error : " + e.Message);
                });

                mXmlSchemaSettings = new XmlReaderSettings();
                mXmlSchemaSettings.Schemas.Add(schema);
                mXmlSchemaSettings.ValidationType = ValidationType.Schema;
            }

            foreach (var res in GetResources(ass, "resources/(.+?)[.]xml"))
            {
                XDocument xmldoc = null;
                try
                {
                    XmlReader reader = XmlReader.Create(res.Value, mXmlSchemaSettings);
        ...
        /// </summary>
        private IDictionary<string, UnmanagedMemoryStream> GetResources(Assembly asm, string filter)
        {
            string resName = asm.GetName().Name + ".g.resources";

            using (var manifestStream = asm.GetManifestResourceStream(resName))
            {
                using (var reader = new ResourceReader(manifestStream))
                {
                    var result = new Dictionary<string, UnmanagedMemoryStream>();
                    ...
                    foreach (DictionaryEntry res in reader)
                    {
                        ...
                        if (Regex.IsMatch(path, filter)) result.Add(path, stream);
                    }
